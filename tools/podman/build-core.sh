...
# Experiment, off unless AUDIO_ASYNC is set. core_constraints.sdc declares every
# clock in the design asynchronous to every other one except the audio PLL's two
# outputs, which are in no group at all, so Quartus times every crossing between
# the audio domain and the rest as a real path and hold-fixes them. They come
# from a separate PLL at an unrelated frequency, so they are asynchronous in
# fact. This measures whether saying so gives back the hold margin that the
# worst path keeps hovering on the wrong side of. Upstream's file is untouched.
if [[ -n "${AUDIO_ASYNC:-}" ]]; then
  cat >> "$SRC/core/core_constraints.sdc" <<'SDC'

set_clock_groups -asynchronous \
 -group { ic|audio_mixer|audio_pll|mf_audio_pll_inst|altera_pll_i|general[0].gpll~PLL_OUTPUT_COUNTER|divclk } \
 -group { ic|audio_mixer|audio_pll|mf_audio_pll_inst|altera_pll_i|general[1].gpll~PLL_OUTPUT_COUNTER|divclk }
SDC
  echo "== audio PLL clocks declared asynchronous (experiment)"
fi
