...
// finished loading).
//
// 32 titles of 20 characters. 20 because the Game Boy screen is 160 pixels and
// the font cell is 8, so that is a full line and there is nowhere to put a
// twenty-first character. Characters are stored as font indices, the ASCII
...
module cheat_titles (
	...
	logic [5:0] ram [0:639];         // 32 titles x 20 characters
	reg   [4:0] len [0:31];

	// group*20 = group*16 + group*4: two shifts and an add. Spelled out because
	// 20 is not a power of two, so the address is not just a concatenation.
	wire [9:0] wr_base = ({5'd0, wr_group} << 4) + ({5'd0, wr_group} << 2);
	wire [9:0] wr_at   = wr_base + {5'd0, wr_col};
	wire [9:0] rd_base = ({5'd0, rd_group} << 4) + ({5'd0, rd_group} << 2);
